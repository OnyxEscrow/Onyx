...
pub fn get_db_encryption_key() -> Result<String> {
    match std::env::var("DB_ENCRYPTION_KEY") {
        Ok(key) => {
            ...
            // Allow empty key for development with unencrypted databases
            // Check for special marker: "unencrypted" or empty with ALLOW_UNENCRYPTED_DB=1
            if key.is_empty() || key == "unencrypted" {
                let allow_unencrypted = std::env::var("ALLOW_UNENCRYPTED_DB")
                    .map(|v| v == "1" || v == "true")
                    .unwrap_or(false);

                if allow_unencrypted || key == "unencrypted" {
                    tracing::warn!(
                        "⚠️  Empty DB_ENCRYPTION_KEY - using UNENCRYPTED database (dev mode only)"
                    );
...
mod tests {
    ...
    fn test_key_reconstruction_programmatic() -> Result<()> {
        ...
        let original_key: &[u8; 32] = b"12345678901234567890123456789012";
