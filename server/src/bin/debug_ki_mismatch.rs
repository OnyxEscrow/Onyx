...
fn main() -> Result<()> {
    ...
    println!("λ_buyer (signer 1) = 2 mod L: {}", hex::encode(&lambda_buyer.to_bytes()[..8]));
    println!("λ_vendor (signer 2) = -1 mod L: {}", hex::encode(&lambda_vendor.to_bytes()[..8]));
    ...
    println!("x_total = d + λ₁*b₁ + λ₂*b₂: {}...", hex::encode(&x_total.to_bytes()[..8]));
    
    // Verify P = d*G + B
    let d_g = &d * ED25519_BASEPOINT_TABLE;
    let p_expected = d_g + group_pubkey;
    println!("\nP_expected = d*G + B: {}", hex::encode(p_expected.compress().as_bytes()));
    ...
    println!("\nx_total * G:          {}", hex::encode(p_from_x.compress().as_bytes()));
    ...
    println!("\n=== PKI AGGREGATION ===");
    println!("buyer_pki + vendor_pki:  {}", hex::encode(sum_pki.compress().as_bytes()));
    ...
    println!("Expected buyer_pki  ((d + λ₁*b₁) * Hp(P)): {}", hex::encode(expected_buyer_pki.compress().as_bytes()));
    ...
    println!("\nExpected vendor_pki (λ₂*b₂ * Hp(P)):       {}", hex::encode(expected_vendor_pki.compress().as_bytes()));
    ...
    println!("Expected buyer_pki  ((d + λ₁*b₁) * Hp(B)): {}", hex::encode(expected_buyer_pki_wrong.compress().as_bytes()));
    println!("Match actual buyer_pki: {}", expected_buyer_pki_wrong == buyer_pki);
    println!("\nExpected vendor_pki (λ₂*b₂ * Hp(B)):       {}", hex::encode(expected_vendor_pki_wrong.compress().as_bytes()));
    println!("Match actual vendor_pki: {}", expected_vendor_pki_wrong == vendor_pki);
