...
//!
//! This binary:
//! 1. Reads escrow funding data from encrypted database
//! 2. Takes FROST secret shares as command-line arguments (from browser localStorage)
//! 3. Fetches ring members from stagenet daemon
//! 4. Computes key image with derivation (v0.54.0)
//! 5. Signs CLSAG using FROST 2-of-3 threshold
//! 6. Builds complete Monero transaction with Bulletproof+ using MoneroTransactionBuilder
//! 7. Broadcasts to daemon
//!
//! Usage: cargo run --release --bin full_offline_broadcast <escrow_id> <buyer_share_hex> <vendor_share_hex> <payout_address> [--broadcast]
...
struct EscrowData {
    ...
    // Aggregated key image from browser (v0.62.0 fix for derivation mismatch)
    aggregated_key_image: Option<String>,
    // v0.70.0: Dispute detection for refund fee calculation
    dispute_signing_pair: Option<String>,
}
...
fn load_escrow_data(conn: &mut SqliteConnection, escrow_id: &str) -> Result<EscrowData> {
    ...
    let query = format!(
        "SELECT id, multisig_view_key, \
         ...
         amount, frost_group_pubkey, aggregated_key_image, dispute_signing_pair \
    ...
    Ok(EscrowData {
        ...
        aggregated_key_image: row.aggregated_key_image,
        dispute_signing_pair: row.dispute_signing_pair,
    })
}
...
/// Check daemon has peer connections before broadcast
/// Returns (outgoing, incoming) connection counts
async fn check_daemon_connectivity(client: &reqwest::Client) -> Result<(u32, u32)> {
    #[derive(Deserialize)]
    ...
    if total == 0 {
        // WARNING only - some public nodes report 0 peers but can still propagate
        // This happens with nodes behind load balancers or special configurations
        println!("⚠️  Daemon reports 0 peer connections (Height: {}, Target: {})",
                 info.height, info.target_height);
        println!("   Proceeding with broadcast anyway (public nodes may still propagate)");
    } else {
        println!("✅ Daemon connectivity OK: {} outgoing, {} incoming peers",
                 info.outgoing_connections_count, info.incoming_connections_count);
    }

    Ok((info.outgoing_connections_count, info.incoming_connections_count))
}
...
async fn broadcast_transaction(client: &reqwest::Client, tx_hex: &str) -> Result<String> {
    ...
    check_daemon_connectivity(client).await?;
...
async fn main() -> Result<()> {
    ...
    if args.len() < 5 {
        println!("Usage: {} <escrow_id> <buyer_share_hex> <vendor_share_hex> <payout_address>", args[0]);
        println!("\nThis binary reads escrow funding data from the encrypted database,");
        println!("takes FROST secret shares from command line (from browser localStorage),");
        println!("and performs complete FROST 2-of-3 CLSAG signing followed by broadcast.");
        println!("\nTo get the shares from browser localStorage, run in console:");
        println!("  JSON.parse(localStorage.getItem('frost_key_package_buyer_<escrow_id>')).secret_share");
        println!("  JSON.parse(localStorage.getItem('frost_key_package_vendor_<escrow_id>')).secret_share");
        return Ok(());
    }

    let escrow_id = &args[1];
    let buyer_share_hex = &args[2];
    let vendor_share_hex = &args[3];
    let payout_address = &args[4];

    println!("╔══════════════════════════════════════════════════════════════════════════╗");
    println!("║     FULL OFFLINE FROST CLSAG BROADCAST                                   ║");
    ...
    println!("Buyer share:  {}...", &buyer_share_hex[..16]);
    println!("Vendor share: {}...", &vendor_share_hex[..16]);
    ...
    if p_computed != p_expected {
        ...
        let lagrange_sum = &(lambda_buyer * b_buyer + lambda_vendor * b_vendor) * ED25519_BASEPOINT_TABLE;
        println!("  (λ₁*b₁ + λ₂*b₂)*G: {}", hex::encode(lagrange_sum.compress().as_bytes()));

        if lagrange_sum != group_pubkey {
            bail!("CRITICAL: Lagrange shares don't match group pubkey! DKG may be incomplete.");
        }
    } else {
        println!("✅ x_total * G = P verified");
    }

    // Key image: KI = x_total * Hp(P)
    // CRITICAL: For CLSAG to close, KI MUST match the signing key (x_total).
    // The browser's aggregated_key_image is computed WITHOUT derivation, so it
    // doesn't match x_total. We MUST compute KI here with the correct private key.
    let hp_p = hash_to_point(p_expected.compress().to_bytes());
    let key_image = x_total * hp_p;
    println!("✅ Key image (computed): {}", hex::encode(key_image.compress().as_bytes()));

    // Log comparison with stored KI if available (for debugging)
    if let Some(ref stored_ki) = escrow.aggregated_key_image {
        let computed_ki_hex = hex::encode(key_image.compress().as_bytes());
        if stored_ki != &computed_ki_hex {
            println!("ℹ️  Note: Stored KI differs (browser computed without derivation)");
            println!("   Stored:   {}...", &stored_ki[..16]);
            println!("   Computed: {}...", &computed_ki_hex[..16]);
            println!("   Using computed KI (matches signing key x_total)");
        }
    }
    ...
    // v0.71.0 FIX: Generate DETERMINISTIC TX secret key (r)
    // CRITICAL: MUST match prepare_sign() in escrow.rs (lines 4049-4054)
    // Using random key caused stealth address mismatch → 100% to vendor, 0% to platform
    let tx_secret_key: [u8; 32] = {
        ...
        tx_secret_hasher.update(&(escrow.amount as u64).to_le_bytes());
        tx_secret_hasher.finalize().into()
    };
    println!("[v0.71.0] Using DETERMINISTIC tx_secret_key (matches prepare_sign)");
    ...
    // Detect refund: dispute_signing_pair == "arbiter_buyer" means buyer gets funds (refund)
    ...
    println!("[v0.70.0] Platform Fee Configuration:");
    ...
    // Step 5: Generate output stealth addresses and commitments
    println!("\n=== STEP 5: Generate Outputs (2 REAL) ===\n");

    // Output 0: Real output to recipient
    ...
    // Commitment for output 0: C = mask*G + amount*H
    ...
    // =========================================================================
    // Commitment Balance with 2 REAL outputs
    // =========================================================================
    ...
    // ==========================================================================
    // BUG FIX v0.72.0: DO NOT adjust mask_1!
    // ==========================================================================
    // Previous code added a random offset to mask_1, which broke the platform
    // wallet's ability to verify the commitment.
    //
    // The wallet derives mask from shared secret: mask = Hs("commitment_mask" || Hs(derivation || i))
    // If we use a different mask for commitment, the wallet's verification fails!
    //
    // CORRECT APPROACH:
    // - pseudo_mask = mask_0 + mask_1 (required for commitment balance)
    // - z_diff = funding_mask - pseudo_mask is naturally non-zero because
    //   funding_mask is independent of mask_0/mask_1
    // - NO random offset needed - removes the bug that broke platform fee!
    // ==========================================================================
    ...
    println!("[v0.72.0-FIX] Using ORIGINAL masks (no random offset)");
    println!("[v0.72.0-FIX] z_diff will be non-zero because funding_mask != pseudo_mask");
    ...
    println!("pseudo_out (adjusted mask): {}", hex::encode(&pseudo_out_bytes));
    ...
    println!("[v0.70.0] Added 2 REAL outputs: recipient ({} atomic) + platform ({} atomic)", recipient_amount, platform_fee);
    ...
    // z_diff = funding_mask - pseudo_mask (naturally non-zero!)
    // This creates a valid D point: D = z_diff * Hp(P) != identity
    // funding_mask is from escrow funding, pseudo_mask = mask_0 + mask_1 from outputs
    ...
    let should_broadcast = args.get(5).map(|s| s == "--broadcast").unwrap_or(false);

    println!("\n╔══════════════════════════════════════════════════════════════════════════╗");
    println!("║                         TRANSACTION READY                                ║");
    println!("╚══════════════════════════════════════════════════════════════════════════╝\n");

    println!("Transaction details (v0.70.0 - Platform Fee):");
    println!("  Escrow ID:      {}", escrow_id);
    println!("  Type:           {}", if is_refund { "REFUND (3% fee)" } else { "RELEASE (5% fee)" });
    ...
    if should_broadcast {
        ...
        println!("   cargo run --release --bin full_offline_broadcast {} {} {} {} --broadcast",
            escrow_id, buyer_share_hex, vendor_share_hex, payout_address);
    }

    Ok(())
}

// NOTE: fetch_ring_indices was removed because it caused Bug #2 (ring indices mismatch).
// Ring indices must come from the same fetch_ring_members() call used for CLSAG signing.
