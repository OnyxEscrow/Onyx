...
fn main() {
    ...
    println!("Hp(G) from our impl:      {}", hex::encode(hp_g.compress().to_bytes()));
    ...
    println!("Hp(P[15]):      {}", hex::encode(hp_ring.compress().to_bytes()));
    ...
    println!("Hp([0;32]):     {}", hex::encode(hp_zeros.compress().to_bytes()));
