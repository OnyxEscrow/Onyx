...
fn complete_signature(
    ...
    let alpha_encrypted =
        hex::decode(&partial_tx.alpha_encrypted).context("Invalid alpha_encrypted hex")?;
...
fn main() -> Result<()> {
    ...
    if args.len() < 5 {
        eprintln!(
            "Usage: {} --escrow-id <id> --spend-key <hex> [--mask-share <hex>]",
            args[0]
        );
        eprintln!();
    ...
    println!(
        "[Signer2] Got partial_tx from DB ({} chars)",
        partial_tx_json.len()
    );

    let partial_tx: PartialTx =
        serde_json::from_str(partial_tx_json).context("Failed to parse partial_tx JSON")?;
    ...
    println!(
        "[Signer2] Completed CLSAG: {}",
        &completed_json[..100.min(completed_json.len())]
    );
