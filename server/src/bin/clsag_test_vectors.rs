...
fn main() {
    ...
    let x1_bytes: [u8; 32] = [
        ...
        0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x00,  // Last byte 0 to ensure < L
    ];
    let x2_bytes: [u8; 32] = [
    ...
    println!("α1 (buyer nonce):        {}", hex::encode(alpha1.as_bytes()));
    println!("α2 (vendor nonce):       {}", hex::encode(alpha2.as_bytes()));

    // Masks
    let z_bytes: [u8; 32] = [  // funding_mask (input commitment mask)
    ...
    println!("pseudo_out_mask:         {}", hex::encode(pseudo_out_mask.as_bytes()));
    println!("mask_delta (z - pom):    {}", hex::encode(mask_delta.as_bytes()));
    ...
    println!("λ1 + λ2:     {} (should be 01000...)", hex::encode(lambda_sum.as_bytes()));
    ...
    println!("\nP1 = x1*G:               {}", hex::encode(p1.compress().as_bytes()));
    println!("P2 = x2*G:               {}", hex::encode(p2.compress().as_bytes()));
    println!("P_agg = x_agg*G:         {}", hex::encode(p_agg.compress().as_bytes()));

    // Verify P_agg = λ1*P1 + λ2*P2
    let p_agg_verify = lambda1 * p1 + lambda2 * p2;
    println!("λ1*P1 + λ2*P2:           {}", hex::encode(p_agg_verify.compress().as_bytes()));
    ...
    println!("\nα_agg = α1 + α2:         {}", hex::encode(alpha_agg.as_bytes()));
    ...
    println!("Hp(P_agg):               {}", hex::encode(hp_p.compress().as_bytes()));

    // Key Image: I = x_agg * Hp(P)
    let key_image = x_agg * hp_p;
    println!("I = x_agg * Hp(P):       {}", hex::encode(key_image.compress().as_bytes()));

    // Partial key images (for aggregation verification)
    let pki1 = x1 * hp_p;
    let pki2 = x2 * hp_p;
    println!("\nPKI1 = x1 * Hp(P):       {}", hex::encode(pki1.compress().as_bytes()));
    println!("PKI2 = x2 * Hp(P):       {}", hex::encode(pki2.compress().as_bytes()));

    // Verify: λ1*PKI1 + λ2*PKI2 = I
    let ki_verify = lambda1 * pki1 + lambda2 * pki2;
    println!("λ1*PKI1 + λ2*PKI2:       {}", hex::encode(ki_verify.compress().as_bytes()));
    println!("Key image matches: {}", key_image == ki_verify);

    // D = mask_delta * Hp(P) - CRITICAL: must not be identity!
    let d = mask_delta * hp_p;
    println!("\nD = mask_delta * Hp(P):  {}", hex::encode(d.compress().as_bytes()));
    ...
    println!("L[{}] = α_agg * G:        {}", real_index, hex::encode(l_real.compress().as_bytes()));

    // R[real] = α_agg * Hp(P)
    let r_real = alpha_agg * hp_p;
    println!("R[{}] = α_agg * Hp(P):    {}", real_index, hex::encode(r_real.compress().as_bytes()));
    ...
    println!("pseudo_out:              {}", hex::encode(pseudo_out.compress().as_bytes()));
    ...
    println!("c[{}]:                    {}", next_index, hex::encode(c_next.as_bytes()));

    // For this test, we'll use c_next as c[real] (simplified)
    // In real CLSAG, you'd propagate through the full ring
    let c_real = c_next;
    println!("c[{}] (for signing):      {}", real_index, hex::encode(c_real.as_bytes()));
    ...
    println!("λ1*mask_delta:           {}", hex::encode(effective_mask_delta_1.as_bytes()));
    println!("s1:                      {}", hex::encode(s1_wasm.as_bytes()));
    ...
    println!("λ2*mask_delta:           {}", hex::encode(effective_mask_delta_2.as_bytes()));
    println!("s2:                      {}", hex::encode(s2_wasm.as_bytes()));

    // Aggregated
    let s_agg_wasm = s1_wasm + s2_wasm;
    println!("\n=== AGGREGATED (WASM style) ===");
    println!("s = s1 + s2 = α_agg - c_p*x_agg - c_c*(λ1+λ2)*mask_delta");
    println!("s_agg (WASM):            {}", hex::encode(s_agg_wasm.as_bytes()));
    ...
    println!("s1_partial:              {}", hex::encode(s1_partial.as_bytes()));
    ...
    println!("s2_contribution:         {}", hex::encode(s2_contribution.as_bytes()));
