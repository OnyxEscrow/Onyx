...
fn compute_aggregation_coefficients(
    ...
    for c in ring_commitments { hasher_p.update(c.compress().as_bytes()); }
    ...
    for c in ring_commitments { hasher_c.update(c.compress().as_bytes()); }
...
fn compute_round_hash(
    ...
    for c in ring_commitments { hasher.update(c.compress().as_bytes()); }
...
fn verify_clsag(
    ...
    let (mu_p, mu_c) = compute_aggregation_coefficients(
        ring_keys, ring_commitments, key_image, d_inv8, pseudo_out  // Use d_inv8 (D/8)!
    );
    ...
    for i in 0..ring_size {
        ...
        if i < 2 || i == ring_size - 1 {
            println!("    c[{}→{}]: {}", i, (i+1) % ring_size, hex::encode(c.as_bytes()));
        }
    }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let height_resp: serde_json::Value = client.post(format!("{}/json_rpc", &get_daemon_url()))
    ...
    let tx_resp: serde_json::Value = client.post(format!("{}/get_transactions", &get_daemon_url()))
    ...
    for _ in 0..output_count {
        let _ = read_varint(&tx, &mut pos);
        let t = tx[pos]; pos += 1;
        if t == 0x03 { pos += 33; } else if t == 0x02 { pos += 32; }
    }
    ...
    for _ in 0..output_count { pos += 8; } // ecdhInfo
    let mut output_commitments: Vec<EdwardsPoint> = Vec::new();
    for _ in 0..output_count { output_commitments.push(read_point(&tx, &mut pos)); }
    ...
    println!("hashes[2] BP+ kv: {} ({} keys)", hex::encode(&bp_hash), bp_kv.len() / 32);
    ...
    for i in 0..input_count as usize {
        ...
        for _ in 0..ring_size { s_values.push(read_scalar(&tx, &mut pos)); }
    ...
    for _ in 0..input_count { pseudo_outs.push(read_point(&tx, &mut pos)); }
    ...
    for i in 0..input_count as usize {
        ...
        for out in ring_resp["outs"].as_array().unwrap_or(&vec![]) {
            let k: [u8; 32] = hex::decode(out["key"].as_str().unwrap_or(""))?.try_into().unwrap();
            let m: [u8; 32] = hex::decode(out["mask"].as_str().unwrap_or(""))?.try_into().unwrap();
        ...
        println!("    pseudo_out: {}", hex::encode(pseudo_outs[i].compress().as_bytes()));
        println!("    ring_commitment[15]: {}", hex::encode(ring_commitments[15].compress().as_bytes()));
        let valid = verify_clsag(&ring_keys, &ring_commitments, &key_images[i], &pseudo_outs[i], &clsag_message, c1, s, d);
