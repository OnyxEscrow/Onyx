...
fn main() {
    ...
    println!(
        "Match first 8: {}",
        if hex::encode(&output_mask[..8]) == "e55cfe3edeb36b96" {
            "✅"
        } else {
            "❌"
        }
    ...
    println!(
        "Match first 8: {}",
        if hex::encode(&pseudo_out_mask[..8]) == "9a8fa101e9dad0bc" {
            "✅"
        } else {
            "❌"
        }
    ...
    println!(
        "Match first 8: {}",
        if hex::encode(&mask_delta[..8]) == "15992b540ed51058" {
            "✅"
        } else {
            "❌"
        }
    ...
    println!(
        "dummy_mask derivation: {}",
        if dummy_match { "✅" } else { "❌" }
    );
    println!(
        "pseudo_out_mask sum: {}",
        if pseudo_match { "✅" } else { "❌" }
    );
    println!(
        "mask_delta computation: {}",
        if delta_match { "✅" } else { "❌" }
    );
