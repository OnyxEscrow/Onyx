...
fn main() -> Result<()> {
    ...
    let (id, addr, view_key, tx_hash, out_idx, global_idx, mask, funding_tx_pk, amount, buyer_sig, vendor_sig, payout_addr, status,
         buyer_pki, vendor_pki, arbiter_pki, aggregated_ki, ring_json,
         vendor_nonce, buyer_nonce, nonce_agg,
         vendor_signed_ts, buyer_signed_ts, first_signer_role) = &result[0];
    ...
    println!("Vendor Nonce Public:  {}", if has_vendor_nonce { "✅ SUBMITTED" } else { "❌ MISSING" });
    println!("Buyer Nonce Public:   {}", if has_buyer_nonce { "✅ SUBMITTED" } else { "❌ MISSING" });
    println!("Nonces Aggregated:    {}", if has_nonce_agg { "✅ YES" } else { "❌ NO" });

    if has_vendor_nonce {
        println!("Vendor Nonce (prefix): {}...", vendor_nonce.as_ref().map(|n| &n[..32.min(n.len())]).unwrap_or(""));
    }
    if has_buyer_nonce {
        println!("Buyer Nonce (prefix):  {}...", buyer_nonce.as_ref().map(|n| &n[..32.min(n.len())]).unwrap_or(""));
    }
    if has_nonce_agg {
        println!("Aggregated Nonce:      {}...", nonce_agg.as_ref().map(|n| &n[..64.min(n.len())]).unwrap_or(""));
    }
    ...
    if let Some(ring) = ring_json {
        ...
        if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(ring) {
            if let Some(obj) = parsed.as_object() {
                ...
                println!("tx_prefix_hash:     {}", if has_tx_prefix { "✅ PRESENT" } else { "❌ MISSING" });
                println!("stealth_address:    {}", if has_stealth { "✅ PRESENT" } else { "❌ MISSING" });
                println!("tx_pubkey:          {}", if has_tx_pubkey { "✅ PRESENT" } else { "❌ MISSING" });
                println!("ring_member_indices: {}", if has_ring_indices { "✅ PRESENT" } else { "❌ MISSING" });
                ...
                if let Some(rpk) = obj.get("ring_public_keys").and_then(|v| v.as_array()) {
                    ...
                    // Get signer_index
                    let signer_idx = obj.get("signer_index")
                        .and_then(|v| v.as_u64())
                        .unwrap_or(15) as usize;

                    println!("P[{}] (signer position): {}", signer_idx,
                        rpk.get(signer_idx)
                            .and_then(|v| v.as_str())
                            .unwrap_or("MISSING"));
                    ...
                    let stealth = obj.get("stealth_address").and_then(|v| v.as_str()).unwrap_or("");
                    let fop = obj.get("funding_output_pubkey").and_then(|v| v.as_str()).unwrap_or("");

                    println!("\n=== P[{}] MATCH CHECK ===", signer_idx);
                    println!("P[{}]:                  {}", signer_idx, p_signer);
                    println!("stealth_address:        {}", stealth);
                    println!("funding_output_pubkey:  {}", fop);
                    println!("P[{}] == stealth_address?       {}", signer_idx,
                        if p_signer == stealth { "✅ YES" } else { "❌ NO" });
                    println!("P[{}] == funding_output_pubkey? {}", signer_idx,
                        if p_signer == fop { "✅ YES" } else { "❌ NO" });
                    println!("stealth == funding_output_pubkey? {}",
                        if stealth == fop { "✅ YES" } else { "❌ NO" });
                }
            }
        } else {
            println!("Ring Data (parse failed): {}...", &ring[..200.min(ring.len())]);
        }
    } else {
    ...
    println!("c1 match:  {}", if buyer_c1 == vendor_c1 { "✅ YES" } else { "❌ NO" });

    println!("\nBuyer D:   {}", buyer_d);
    println!("Vendor D:  {}", vendor_d);
    println!("D match:   {}", if buyer_d == vendor_d { "✅ YES" } else { "❌ NO" });
    ...
    if !buyer_s.is_empty() && !vendor_s.is_empty() && buyer_s.len() == vendor_s.len() {
        ...
        for i in 0..buyer_s.len() {
            ...
            println!("s[{:2}]: {} {} {}", i,
                ...
                if !matches { format!("!= {}", &vendor_s[i][..16.min(vendor_s[i].len())]) } else { String::new() }
            );
        }
        if all_match {
    ...
    if buyer_s.len() > 15 {
        ...
        println!("Vendor s[15]: {}", vendor_s.get(15).unwrap_or(&"N/A".to_string()));
    }
