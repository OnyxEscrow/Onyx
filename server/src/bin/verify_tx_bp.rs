...
fn main() {
    ...
    for i in 0..output_count {
        ...
        match CompressedEdwardsY(arr).decompress() {
            ...
            None => println!("outPk[{}]: {} ❌ INVALID POINT!", i, hex::encode(commitment)),
        }
        pos += 32;
    }
    ...
    let a_point = &tx[pos..pos+32]; pos += 32;
    let a1_point = &tx[pos..pos+32]; pos += 32;
    let b_point = &tx[pos..pos+32]; pos += 32;
    let r1_scalar = &tx[pos..pos+32]; pos += 32;
    let s1_scalar = &tx[pos..pos+32]; pos += 32;
    let d1_scalar = &tx[pos..pos+32]; pos += 32;
    ...
    let bp_end = pos;
    println!("\nBP+ total size: {} bytes (pos {} to {})", bp_end - bp_start, bp_start, bp_end);
    ...
    println!("Expected rounds for {} outputs: ~{}", output_count, expected_rounds);
