...
fn get_known_shares(escrow_id: &str) -> Option<([u8; 32], [u8; 32])> {
    ...
    if escrow_id.starts_with("ef57f177") {
        let buyer_share = hex::decode("916e1d306297b252a49d616846bc1e22276ea3d535280bdde3f8d8123541b70b")
            .ok()?;
        let vendor_share = hex::decode("7dfcdfcaafbe5b7abbb69237954839f30172c31d91bbfe57357542bfd504b60e")
            .ok()?;
...
impl CryptoValidator {
    ...
    fn load_escrow_data(&mut self, conn: &rusqlite::Connection) -> Result<()> {
        ...
        if let Some((buyer_share, vendor_share)) = known_shares {
            ...
            println!("  [INFO] Using known shares for escrow {}...", &self.escrow_id[..8]);
        } else {
        ...
        println!("  Amount: {} piconero ({:.12} XMR)", row.2, row.2 as f64 / 1e12);
    ...
    fn validate_one_time_address(&mut self) -> Result<()> {
        ...
        let tx_pubkey = match self.tx_pubkey {
            Some(p) => p,
            None => {
                self.add_result("2. One-time address", "tx_pubkey required", "MISSING", false);
                return Ok(());
            }
        };
        ...
        let group_pubkey = match self.group_pubkey {
            Some(g) => g,
            None => {
                self.add_result("2. One-time address", "group_pubkey required", "MISSING", false);
                return Ok(());
            }
        };
        ...
        println!("  Shared secret (8*v*R): {}...", hex::encode(&shared_secret_bytes[..8]));
        ...
        println!("  Hs(derivation||idx): {}...", hex::encode(&derivation_scalar_bytes[..8]));
        ...
        println!("  One-time address P: {}", hex::encode(self.one_time_address.unwrap()));
    ...
    fn validate_pki_computation(&mut self) -> Result<()> {
        ...
        let one_time = match self.one_time_address {
            Some(p) => p,
            None => {
                self.add_result("3. PKI computation", "one_time_address required", "MISSING", false);
                return Ok(());
            }
        };
        ...
        println!("  Hp(P): {}...", hex::encode(&hp.compress().to_bytes()[..8]));
        ...
        if let (Some(buyer_share), Some(vendor_share)) = (self.buyer_share, self.vendor_share) {
            ...
            println!("  Expected PKI_vendor (d + λ_v*s_v)*Hp: {}...",
                     hex::encode(&pki_vendor_expected.compress().to_bytes()[..8]));
            println!("  Expected PKI_buyer (λ_b*s_b)*Hp: {}...",
            ...
            if let Some(stored_vendor) = self.vendor_pki {
                ...
                println!("  Stored PKI_vendor: {}...", hex::encode(&stored_vendor[..8]));
                println!("  VENDOR PKI MATCH: {}", if matches { "✓" } else { "✗" });
                self.add_result(
                    "3a. Vendor PKI",
                    &format!("{}...", hex::encode(&pki_vendor_expected.compress().to_bytes()[..8])),
            ...
            if let Some(stored_buyer) = self.buyer_pki {
                ...
                self.add_result(
                    "3b. Buyer PKI",
                    &format!("{}...", hex::encode(&pki_buyer_expected.compress().to_bytes()[..8])),
            ...
            println!("\n  Expected aggregated KI: {}...", hex::encode(&expected_ki.compress().to_bytes()[..8]));
            ...
            if let Some(stored_ki) = self.aggregated_ki {
                ...
                println!("  Stored aggregated KI: {}...", hex::encode(&stored_ki[..8]));
                println!("  AGGREGATED KI MATCH: {}", if matches { "✓" } else { "✗" });
                self.add_result(
                    "3c. Aggregated Key Image",
                    &format!("{}...", hex::encode(&expected_ki.compress().to_bytes()[..8])),
            ...
            println!("  Aggregated == Theoretical: {}", if ki_matches_theory { "✓" } else { "✗" });
        } else {
    ...
    fn validate_commitment_balance(&mut self) -> Result<()> {
        ...
        let pseudo_out = match self.pseudo_out {
            Some(p) => p,
            None => {
                self.add_result("4. Commitment balance", "pseudo_out required", "MISSING", false);
        ...
        println!("  payout_amount: {} piconero ({:.12} XMR)", payout_amount, payout_amount as f64 / 1e12);

        // Verify: input_amount should equal payout + fee
        let amount_balance = self.escrow_amount == payout_amount + self.fee;
        println!("  Amount balance (in = out + fee): {}", if amount_balance { "✓" } else { "✗" });
        self.add_result(
            "4a. Amount balance",
        ...
        println!("  Derived mask*G component: {}...", hex::encode(&mask_g.compress().to_bytes()[..8]));
    ...
    fn validate_clsag_structure(&mut self) -> Result<()> {
        ...
        for (name, sig_opt) in [("buyer", &self.buyer_signature), ("vendor", &self.vendor_signature)] {
            if let Some(sig_json) = sig_opt {
                sig_count += 1;
                if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(sig_json) {
                    if let Some(sig) = parsed.get("signature") {
                        let c1 = sig.get("c1").and_then(|v| v.as_str()).unwrap_or("MISSING");
                        let d = sig.get("D").or(sig.get("d")).and_then(|v| v.as_str()).unwrap_or("MISSING");
                        let s_values = sig.get("s").and_then(|v| v.as_array()).map(|a| a.len()).unwrap_or(0);
                        ...
                        if let Some(s_array) = sig.get("s").and_then(|v| v.as_array()) {
                            let valid_s = s_array.iter().all(|s| {
                                s.as_str().map(|h| h.len() == 64).unwrap_or(false)
                            });
                            println!("    s-values format valid: {}", if valid_s { "✓" } else { "✗" });
                        }
                    }
                }
            }
        ...
        self.add_result("5a. Both signatures present", "2", &format!("{}", sig_count), sig_ok);
    ...
    fn validate_ring_data(&mut self) -> Result<()> {
        ...
        if let Some(ref ring_json) = self.ring_data_json {
            if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(ring_json) {
                let ring_size = parsed.get("ring_public_keys")
                    .and_then(|v| v.as_array())
                    .map(|a| a.len())
                    .unwrap_or(0);
                let commitment_count = parsed.get("ring_commitments")
                    .and_then(|v| v.as_array())
                    .map(|a| a.len())
                    .unwrap_or(0);
                let signer_index = parsed.get("signer_index")
                    .and_then(|v| v.as_u64())
                    .unwrap_or(0);
                let tx_prefix_hash = parsed.get("tx_prefix_hash")
                    .and_then(|v| v.as_str())
                    .unwrap_or("MISSING");
                ...
                println!("  TX prefix hash: {}...", &tx_prefix_hash[..16.min(tx_prefix_hash.len())]);
    ...
    fn print_summary(&self) {
        ...
        if passed == total {
            ...
            println!("  {} CHECKS FAILED - Fix issues before broadcast", total - passed);
        }
        println!("============================================================\n");
    }
}
