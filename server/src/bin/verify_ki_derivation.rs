...
fn main() {
    ...
    println!("\nP_computed: {}", hex::encode(p_computed.compress().as_bytes()));
    println!("P_expected: {}", p_expected_hex);
    
    let p_expected_bytes: [u8; 32] = hex::decode(p_expected_hex).unwrap().try_into().unwrap();
    let p_expected = CompressedEdwardsY(p_expected_bytes).decompress().expect("Invalid P");
    ...
    println!("Hp(P_bytes): {}", hex::encode(hp_direct.compress().as_bytes()));
    let ki_direct = x_total * hp_direct;
    println!("KI direct:   {}", hex::encode(ki_direct.compress().as_bytes()));
}
