...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let height_resp: serde_json::Value = client.post(format!("{}/json_rpc", daemon_url))
    ...
    for h in (height-100..height).rev() {
        let block_resp: serde_json::Value = client.post(format!("{}/json_rpc", daemon_url))
            .json(&serde_json::json!({
                "jsonrpc": "2.0",
                "id": "0", 
                "method": "get_block",
    ...
    let tx_resp: serde_json::Value = client.post(format!("{}/get_transactions", daemon_url))
        .json(&serde_json::json!({
            "txs_hashes": [&tx_hash],
    ...
    println!("TX hex length: {} chars ({} bytes)", tx_hex.len(), tx_hex.len() / 2);
    ...
    for i in 0..input_count {
        ...
        if input_type == 0x02 {
            ...
            println!("Input {}: ring_size={}, key_image={}...", 
                i, key_offset_count, hex::encode(&ki[..8]));
        }
    }
    ...
    for i in 0..output_count {
        ...
        if output_type == 0x03 { // tagged key
    ...
    for i in 0..input_count as usize {
        ...
        println!("  D: {} (valid: {})", hex::encode(&d_bytes), d_point.is_some());
        ...
        let ring_resp: serde_json::Value = client.post(format!("{}/get_outs", daemon_url))
            .json(&serde_json::json!({
    ...
    for i in 0..input_count as usize {
        ...
        println!("pseudo_out[{}]: {}", i, hex::encode(pseudo.compress().as_bytes()));
    }
