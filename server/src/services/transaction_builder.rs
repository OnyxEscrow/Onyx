...
impl MoneroTransactionBuilder {
    ...
    pub fn attach_fcmp_proof(
        ...
        for (i, proof) in data.input_proofs.iter().enumerate() {
            if proof.o_tilde == [0u8; 32] {
                return Err(TransactionBuildError::FcmpError(format!(
                    "Input {}: O~ is zero (identity point)",
                    i
                )));
            }
            if proof.i_tilde == [0u8; 32] {
                return Err(TransactionBuildError::FcmpError(format!(
                    "Input {}: I~ is zero (identity point)",
                    i
                )));
            }
        }
    ...
    fn serialize_prefix(&self, out: &mut Vec<u8>) -> Result<(), TransactionBuildError> {
        ...
        let version = if self.fcmp_data.is_some() {
            3u64
        } else {
            self.version as u64
        };
    ...
    fn serialize_rct_prunable_fcmp(&self, out: &mut Vec<u8>) -> Result<(), TransactionBuildError> {
        let fcmp = self
            .fcmp_data
            .as_ref()
            .ok_or_else(|| TransactionBuildError::FcmpError("FCMP data not attached".into()))?;
    ...
    pub fn validate_tx_bytes(data: &[u8]) -> TxValidationResult {
        ...
        if version != 2 && version != 3 {
            ...
            result.errors.push(format!(
                "Invalid TX version {}, expected 2 (CLSAG) or 3 (FCMP++)",
                version
            ));
        }
        ...
        for _ in 0..num_inputs {
            ...
            if input_type == 0x02 {
                ...
                result
                    .errors
                    .push(format!("Unknown input type 0x{:02x}", input_type));
                return result;
            }
        }
        ...
        for i in 0..num_outputs {
            ...
            match output_type {
                ...
                _ => {
                    ...
                    result
                        .errors
                        .push(format!("Output {}: unknown type 0x{:02x}", i, output_type));
                    return result;
                }
            }
        }
