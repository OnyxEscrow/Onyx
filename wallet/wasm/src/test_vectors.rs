...
pub fn print_validation_results(results: &[CheckpointResult]) {
    ...
    if all_pass {
        ...
        println!("❌ DIVERGENCE DETECTED at: {}", first_fail.unwrap_or("unknown"));
    }
    println!("══════════════════════════════════════════════════════════════════");
}
...
mod tests {
    ...
    fn test_clsag_checkpoints() {
        ...
        for result in &results {
            assert!(result.matches, "Checkpoint '{}' failed:\n  Expected: {}\n  Actual: {}",
    ...
    fn test_lagrange_coefficients() {
        ...
        assert_eq!(hex::encode(sum.as_bytes()),
    ...
    fn test_key_image_aggregation() {
        ...
        assert_eq!(key_image.compress(), ki_from_partials.compress(),
