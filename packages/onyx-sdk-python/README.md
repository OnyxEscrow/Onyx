...
Official Python SDK for the Onyx Escrow-as-a-Service API. Create non-custodial Monero escrows with browser-based FROST multisig.

## Installation

```bash
pip install onyx-escrow
```
...
```python
import asyncio
from onyx_escrow import OnyxClient, EscrowStatus

async def main():
    async with OnyxClient(api_key="nxs_your_api_key") as client:
        # Create an escrow
...
```python
from onyx_escrow import WebhookManager, OnyxWebhookVerificationError

@app.post("/webhooks/onyx")
async def handle_webhook(request: Request):
    payload = await request.body()
    signature = request.headers.get("X-Onyx-Signature", "")
    timestamp = request.headers.get("X-Onyx-Timestamp")
    ...
    except OnyxWebhookVerificationError:
...
except OnyxAuthenticationError:
    print("Invalid API key")
except OnyxValidationError as e:
    print(f"Validation error: {e.message}")
    print(f"Details: {e.details}")
except OnyxRateLimitError as e:
...
client = OnyxClient(
    api_key="nxs_...",
    base_url="https://api.onyx-escrow.com",  # Default
