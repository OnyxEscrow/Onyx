"""Webhook management for Onyx SDK.
...
from .exceptions import OnyxWebhookVerificationError
from .types import (
...
class WebhookManager:
    ...
    def verify_signature(
        ...
        *,
        ...
        It verifies that a webhook payload was signed by Onyx.

        Args:
            payload: The raw request body bytes.
            signature: The X-Onyx-Signature header value.
            secret: Your webhook signing secret.
            timestamp: The X-Onyx-Timestamp header (optional).
        ...
        Raises:
            OnyxWebhookVerificationError: If verification fails.
        """
        import time
        ...
        if timestamp:
            try:
                ...
                if abs(now - ts) > tolerance_seconds:
                    raise OnyxWebhookVerificationError(
                        f"Timestamp too old (>{tolerance_seconds}s)"
                    )
            except ValueError as err:
                raise OnyxWebhookVerificationError("Invalid timestamp format") from err
        ...
        if not hmac.compare_digest(expected, provided):
            raise OnyxWebhookVerificationError("Signature mismatch")
