...
from onyx_escrow.webhooks import WebhookManager

# Test constants
TEST_API_KEY = "nxs_test_1234567890abcdef"
TEST_BASE_URL = "https://api.test.onyx-escrow.com"
...
class TestOnyxClientInit:
    ...
    def test_init_invalid_api_key_format(self) -> None:
        ...
        with pytest.raises(ValueError, match="Invalid API key format"):
            OnyxClient(api_key="invalid_key_123")

    def test_init_custom_base_url(self) -> None:
        """Client accepts custom base URL."""
        client = OnyxClient(api_key=TEST_API_KEY, base_url=TEST_BASE_URL)
        assert client._base_url == TEST_BASE_URL

    def test_init_strips_trailing_slash(self) -> None:
        """Client strips trailing slash from base URL."""
        client = OnyxClient(api_key=TEST_API_KEY, base_url=f"{TEST_BASE_URL}/")
        assert client._base_url == TEST_BASE_URL
...
class TestOnyxClientContextManager:
    """Tests for async context manager."""

    @pytest.mark.asyncio
    async def test_context_manager_closes_client(self) -> None:
        """Client closes HTTP connection when exiting context."""
        async with OnyxClient(api_key=TEST_API_KEY) as client:
...
class TestEscrowOperations:
    ...
    async def test_create_escrow_success(self) -> None:
        ...
        async with OnyxClient(
            api_key=TEST_API_KEY, base_url=TEST_BASE_URL
        ) as client:
            from onyx_escrow.types import CreateEscrowResponse
...
class TestErrorHandling:
    ...
    async def test_authentication_error(self) -> None:
        """401 response raises OnyxAuthenticationError."""
        respx.get(f"{TEST_BASE_URL}/api/v1/escrows/{TEST_ESCROW_ID}").mock(
            return_value=httpx.Response(
                401,
    ...
    async def test_authorization_error(self) -> None:
        """403 response raises OnyxAuthorizationError."""
        ...
        async with OnyxClient(
            ...
            with pytest.raises(OnyxAuthorizationError):
                from onyx_escrow import DisputeResolution
    ...
    async def test_validation_error(self) -> None:
        """422 response raises OnyxValidationError."""
        respx.post(f"{TEST_BASE_URL}/api/v1/escrows/create").mock(
            return_value=httpx.Response(
                422,
                json={
        ...
        async with OnyxClient(
            ...
            with pytest.raises(OnyxValidationError) as exc_info:
    ...
    async def test_rate_limit_error(self) -> None:
        """429 response raises OnyxRateLimitError."""
        respx.get(f"{TEST_BASE_URL}/api/v1/user/escrows").mock(
            return_value=httpx.Response(
                429,
        ...
        async with OnyxClient(
            ...
            with pytest.raises(OnyxRateLimitError) as exc_info:
...
class TestWebhookSignatureVerification:
    ...
    def test_verify_signature_invalid(self) -> None:
        ...
        from onyx_escrow import OnyxWebhookVerificationError

        payload = b'{"type": "escrow.funded"}'

        with pytest.raises(OnyxWebhookVerificationError, match="Signature mismatch"):
    ...
    def test_verify_signature_old_timestamp(self) -> None:
        ...
        with pytest.raises(OnyxWebhookVerificationError, match="Timestamp too old"):
            WebhookManager.verify_signature(
                payload=payload,
...
class TestDkgOperations:
    ...
    async def test_dkg_init(self) -> None:
        ...
        async with OnyxClient(api_key=TEST_API_KEY, base_url=TEST_BASE_URL) as client:
...
class TestAccountOperations:
    ...
    async def test_health_check_failure(self) -> None:
        ...
        async with OnyxClient(
            ...
            with pytest.raises(OnyxConnectionError):
