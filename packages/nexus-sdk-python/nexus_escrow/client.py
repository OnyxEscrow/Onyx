"""NEXUS Escrow API Client.

Main client for interacting with the NEXUS Escrow-as-a-Service API.
"""
...
class NexusClient:
    """Async client for the NEXUS Escrow-as-a-Service API.

    Provides access to all NEXUS API endpoints including escrow management,
    ...
    Alternatively, manage the lifecycle manually:
        ...
        client = NexusClient(api_key="nxs_...")
        try:
            escrow = await client.escrows.create(...)
        finally:
            await client.close()
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            api_key: Your NEXUS API key (starts with "nxs_").
            base_url: Base URL for the API (default: https://api.nexus.io).
        ...
        self._client = httpx.AsyncClient(
            ...
            headers={
                ...
                "User-Agent": f"nexus-python-sdk/{SDK_VERSION}",
            },
        )
    ...
    async def _request(
        ...
        *,
        ...
        Raises:
            NexusApiError: On API error.
            NexusConnectionError: On connection failure.
            NexusTimeoutError: On request timeout.
        """
        ...
        for attempt in range(self._max_retries):
            try:
                ...
                last_error = NexusConnectionError(f"Connection failed: {e}")
                raise last_error from e

            except httpx.TimeoutException as e:
                last_error = NexusTimeoutError(f"Request timed out: {e}")
                if attempt == self._max_retries - 1:
                    raise last_error from e
                continue

            except NexusServerError as e:
                last_error = e
                if attempt == self._max_retries - 1:
                    raise
                continue
        ...
        if last_error:
            raise last_error
        raise NexusConnectionError("Request failed after retries")
    ...
    def _handle_response(self, response: httpx.Response) -> dict[str, Any]:
        ...
        Raises:
            NexusApiError: On error response.
        """
        ...
        if status == 401:
            raise NexusAuthenticationError(message=message, request_id=request_id)

        if status == 403:
            raise NexusAuthorizationError(message=message, request_id=request_id)
    ...
    async def health_check(self) -> bool:
        """Check if the NEXUS API is healthy.

        Returns:
            True if the API is operational.

        Raises:
            NexusConnectionError: If cannot connect to API.
        """
        try:
            response = await self._client.get("/api/health")
            return response.is_success
        except httpx.HTTPError as e:
            raise NexusConnectionError(f"Health check failed: {e}") from e
