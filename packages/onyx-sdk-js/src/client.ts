 ...
 * Onyx SDK Client
 *
 * Main entry point for the Onyx Escrow-as-a-Service API.
 * Provides access to all API resources: escrow, FROST DKG/signing,
 * webhooks, API keys, fees, analytics, and E2EE chat.
 */

import type { OnyxClientConfig, ApiErrorResponse } from './types.js';
 ...
 * @example
 * ```typescript
 * import { OnyxClient } from '@onyx-escrow/sdk';
 *
  ...
  private handleErrorResponse(
    ...
    if (data?.error) {
      ...
      switch (response.status) {
        ...
        default:
          return OnyxApiError.fromResponse(data, response.status);
      }
    }
