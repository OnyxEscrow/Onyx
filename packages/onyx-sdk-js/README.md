...
TypeScript SDK for the Onyx Escrow-as-a-Service (EaaS) API. Non-custodial Monero escrow with FROST 2-of-3 threshold signing.
...
```bash
npm install @onyx-escrow/sdk
# or
yarn add @onyx-escrow/sdk
# or
pnpm add @onyx-escrow/sdk
```

## Quick Start

```typescript
import { OnyxClient, xmrToPiconero } from '@onyx-escrow/sdk';

const client = new OnyxClient({
  apiKey: process.env.ONYX_API_KEY!,
  baseUrl: 'https://your-onyx-instance.com',
});
...
const client = new OnyxClient({
  ...
  baseUrl: 'https://api.onyx-escrow.com', // Optional
...
const { webhook, secret } = await client.webhooks.create({
  url: 'https://example.com/webhooks/onyx',
...
```typescript
import { verifyWebhookSignature, parseWebhookPayload } from '@onyx-escrow/sdk';

app.post('/webhooks/onyx', async (req, res) => {
  const isValid = await verifyWebhookSignature(
    req.body,
    req.headers['x-onyx-signature'],
    req.headers['x-onyx-timestamp'],
...
```typescript
import { xmrToPiconero, piconeroToXmr, formatXmr, isValidMainnetAddress } from '@onyx-escrow/sdk';
...
try {
  ...
  } else if (error instanceof OnyxApiError) {
...
```typescript
import { EscrowResource, xmrToPiconero } from '@onyx-escrow/sdk/escrow';
import { verifyWebhookSignature } from '@onyx-escrow/sdk/webhooks';
import { FrostResource } from '@onyx-escrow/sdk/frost';
import { FeeResource } from '@onyx-escrow/sdk/fees';
import { AnalyticsResource } from '@onyx-escrow/sdk/analytics';
import { ChatResource } from '@onyx-escrow/sdk/chat';
```
